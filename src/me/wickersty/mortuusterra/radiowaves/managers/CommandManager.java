	...
	public void processCommand(Player player, Command cmd, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("mtr")) {
			...
			if ((player.isOp() == true || instance.getPermissionsManager().playerHasPermissions(player, "mtr.admin") || instance.getPermissionsManager().playerHasPermissions(player, "mtr.activate")) && args[0].equalsIgnoreCase("activate")) {
				...
				if (instance.getRadioBroadcastingTowerManager().getRadioBroadcastingTowerByLocation(player.getEyeLocation().getBlock().getLocation()) != null) {
				...
				if (args[1].equalsIgnoreCase("radiotower")) {
					...
					if (!(player.getEyeLocation().getBlock().getType().equals(Material.LEVER))) {
					...
					RadioBroadcastingTower radioTower = new RadioBroadcastingTower(instance, instance.getFrequencyManager().getCitizensBandRadio().getFrequencyRange().get(0), player.getName(), player.getEyeLocation().getBlock().getLocation(), false);
